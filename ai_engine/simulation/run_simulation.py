...
class TrafficSimulationApp:
    ...
    def update(self, dt: float):
        ...
        # Calculate active approach queue PCU for early gap-out detection
        cur_phase_str = str(self.signals.current_phase)
        active_pcu = 99.0
        if "N_" in cur_phase_str: active_pcu = pcu_by_dir.get('N', 0.0)
        elif "S_" in cur_phase_str: active_pcu = pcu_by_dir.get('S', 0.0)
        elif "E_" in cur_phase_str: active_pcu = pcu_by_dir.get('E', 0.0)
        elif "W_" in cur_phase_str: active_pcu = pcu_by_dir.get('W', 0.0)
        elif "EW_THROUGH" in cur_phase_str: active_pcu = pcu_by_dir.get('E', 0.0) + pcu_by_dir.get('W', 0.0)
        elif "NS_THROUGH" in cur_phase_str: active_pcu = pcu_by_dir.get('N', 0.0) + pcu_by_dir.get('S', 0.0)

        switched, completed_p, new_p, allocated = self.signals.update(
            dt, next_decision_calc_fn=self.calculate_next_decision, active_queue_pcu=active_pcu
        )
