    ...
    def __init__(self, vehicle_id: int, vehicle_type: str, direction: str, lane_idx: int = 0, turn_intent: str = None):
        ...
        if turn_intent is None:
            if self.vehicle_type == "ambulance":
                ...
                # Dedicated Inner Left-Turn Lane (90% Left, 10% Straight)
                self.turn_intent = random.choices(["LEFT", "STRAIGHT"], weights=[0.90, 0.10])[0]
            else:
                # Dedicated Outer Straight/Right Lane (70% Straight, 30% Right)
                self.turn_intent = random.choices(["STRAIGHT", "RIGHT"], weights=[0.70, 0.30])[0]
        else:
