...
def run_1hour_simulation_with_comparison():
    ...
    saved_final_path = excel_path
    try:
        wb.save(excel_path)
        print(f"\nOPTIMIZED 1-HOUR REPORT WITH COMPARISON SHEET SAVED TO:\n{excel_path}")
    except PermissionError:
        fallback_path = os.path.join(output_dir, "simulation_1hour_U150_D40_L35_R30_OPTIMIZED.xlsx")
        wb.save(fallback_path)
        saved_final_path = fallback_path
        print(f"\nNOTE: Primary file was open in Excel. Successfully saved to:\n{fallback_path}")
